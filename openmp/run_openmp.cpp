...
// ++++++++ VARIABLES ++++++++ //
#ifndef TEST_MODE
std::string output_positions = "openmp_positions.csv";
std::string output_timing = "openmp_timing.csv";
#endif


// ++++++++ FUNCTIONS ++++++++ //
void compute_acceleration_openmp(std::vector<Body>& bodies) {
...
void check_collisions_openmp(std::vector<Body>& bodies) {
...
void integrate_openmp(std::vector<Body>& bodies, double dt) {
    // Velocity Verlet integration

    // Set n to avoid redundant accesses to bodies.size()
    ...
    compute_acceleration_openmp(bodies);
    for (size_t i = 0; i < n; ++i) {
        if (!bodies[i].alive) continue;
...
int main() {
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        integrate_openmp(bodies, dt);
        check_collisions_openmp(bodies);
